...
function computeGaugeMax(labs = []) {
  ...
  // 너무 작으면 게이지가 민감해지니 최소값 보정 + 보기 좋은 단위로 올림
  const COST_MANWON = Math.max(200, roundUp(maxCost * 1.15, 50)); // 최대치의 115%를 50단위로 올림
  const LEAD_DAYS = Math.max(30, roundUp(maxLead * 1.15, 5));     // 최대치의 115%를 5일 단위로 올림
...
const ScoreModal = memo(function ScoreModal({ lab, onClose, gaugeMax }) {
  ...
  // ✅ 총점 = 인증성공확률(%)
  const totalSuccessPct = useMemo(() => pastSuccessPct, [pastSuccessPct]);
      ...
      <div className="relative w-[min(560px,92vw)] bg-white rounded-3xl border border-gray-200 shadow-2xl overflow-hidden">
        ...
        <div className="p-6">
          ...
          <div className="mb-4 p-4 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-between">
            <div className="text-sm font-bold text-blue-700">인증성공확률</div>
            <div className="text-2xl font-black text-blue-700">{totalSuccessPct}%</div>
          </div>

          <ScoreRow
            label="비용"
            hint="낮을수록 유리"
            display={`${Math.round(costManwon)}만원`}
            barPct={costBarPct}
          />
          <ScoreRow
            label="리드타임"
            hint="낮을수록 유리"
            display={`${Math.round(leadDays)}일`}
            barPct={leadBarPct}
          />
          <ScoreRow
            label="과거 인증성공률"
...
const LabsView = memo(function LabsView({ targetCountry, setTargetCountry, repositoryFiles = [] }) {
  ...
  const gaugeMax = useMemo(() => {
    const base = Array.isArray(matchedLabs) && matchedLabs.length ? matchedLabs : Array.isArray(LABS_DATA) ? LABS_DATA : [];
