...
const RepositoryView = memo(function RepositoryView({
  ...
  const checklist = useMemo(() => {
    ...
    for (const up of uploadedAllowedNames) {
      ...
      if (!matchedMap[n]) matchedMap[n] = up;
    }
      ...
      <div className="relative w-[min(920px,92vw)] max-h-[84vh] bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden">
        <div className="p-6 border-b border-gray-100 bg-gray-50/60 flex items-start justify-between gap-4">
          <div>
            ...
            <div className="text-xs text-gray-500 mt-1">
              아래 10개 문서명과 업로드 파일명이 (확장자 제외 / 공백 정리 후) 일치할 때 체크됩니다.
            </div>
          </div>
          <button
            onClick={() => setChecklistOpen(false)}
        ...
        <div className="px-6 pb-6 overflow-y-auto custom-scrollbar max-h-[60vh]">
          ...
          <div className="mt-4 text-[11px] text-gray-500">
            * 미국(US) 체크리스트는 현재 제외되어 EU 10개 문서만 기준으로 계산합니다.
          </div>
        </div>
      </div>
    </div>
      ...
      <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            {isPicker
              ? "저장소에서 파일을 선택하세요."
              : "파일을 이 영역에 드래그&드롭하거나, 오른쪽 상단 버튼으로 업로드할 수 있습니다."}
          </p>
        </div>
      ...
      <div className="p-6 flex-1 min-h-0 overflow-hidden">
        {!isPicker ? (
          <div
            ...
            onDrop={onDrop}
            className="relative h-full rounded-3xl border border-gray-200 overflow-hidden flex flex-col bg-white"
          >
            {/* ✅ 드래그중일 때만 은은한 오버레이 (UI는 깔끔 유지) */}
            {dragOver && (
              <div className="absolute inset-0 z-20 pointer-events-none bg-blue-50/40 ring-2 ring-blue-200 rounded-3xl" />
            )}
  ...
  <div className="flex items-center gap-2">
    ...
    <span className="text-sm font-black text-gray-800 truncate">
      {r.name}
            ...
            {/* ✅ 아주 얇은 힌트(원하면 삭제 가능) */}
            <div className="px-5 py-3 border-t border-gray-100 text-[11px] text-gray-500 bg-white">
              이 영역에 파일을 드래그&드롭하여 업로드할 수 있습니다.
            </div>
          </div>
        ) : (
          // picker 모드
